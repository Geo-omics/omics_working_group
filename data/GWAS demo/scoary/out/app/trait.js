 ...
 *  - valuesData.isBinary            binary trait data is available
 *  - valuesData.isNumeric           numeric trait data is available
 *  - valuesData.isolateToBinary     {"isolate": boolean}  (only if isBinary)
...
const valuesPromise = configPromise.then(config => {
    return Papa.execPromise(`traits/${trait}/values.tsv`, {
        ...
        valuesData.isBinary = valuesData.meta.fields.includes('binary')
        valuesData.isNumeric = valuesData.meta.fields.includes('numeric')
        console.log('valuesData', valuesData)
        console.log('isBinary', valuesData.isBinary)
        console.log('isNumeric', valuesData.isNumeric)
        if (valuesData.isBinary) {
            valuesData.isolateToBinary = Object.fromEntries(valuesData.data.map(x => [x['isolate'], x['binary'].toLowerCase()]))
        } else {
            document.getElementById('pie-container').remove()
        }

        if (valuesData.isNumeric) {
            valuesData.isolateToValue = Object.fromEntries(valuesData.data.map(x => [x['isolate'], x['numeric']]))
            const vals = valuesData.data.map(x => x['numeric'])
            ...
            valuesData.isolateToNormValue = Object.fromEntries(valuesData.data.map(
                x => [x['isolate'], (x['numeric'] - valMin) / valDiff]
            ))
...
const coverageMatrixPromise = Promise.all(
    ...
    deSanitizeDict['binary'] = 'Binary'
    deSanitizeDict['numeric'] = 'Numeric'
    ...
    // add binary column
    if (valuesData.isBinary) {
        cmValues.meta.fields.push('binary')
        for (const isolateData of cmValues.data) {
            isolateData['binary'] = valuesData.isolateToBinary[isolateData['Isolate']]
        }
    }

    // add numeric column
    if (valuesData.isNumeric) {
        cmValues.meta.fields.push('numeric')
        for (const isolateData of cmValues.data) {
            isolateData['numeric'] = valuesData.isolateToValue[isolateData['Isolate']]
        }
    }
        ...
        const genesPopoverContent = function () {
            ...
            if (valuesData.isBinary) {
                for (const isolateData of cmValues.geneList) {
                    ...
                    const traitPositive = valuesData.isolateToBinary[isolate]
...
const treePromise = Promise.all([documentReadyPromise, newickPromise, valuesPromise, configPromise])
    .then(([_, newickTree, valuesData, config]) => {
        ...
        let blocks = ['gene']
        if (valuesData.isBinary) blocks.push('trait')
        if (valuesData.isNumeric) blocks.push('numeric')
...
/**
 * Update the metadata heatmap of the phylogenetic tree.
 *
 * Based on https://www.phylocanvas.gl/examples/styling-nodes.html
 */
...
const calcOrthogeneData = (gene, coverageMatrix, valuesData) => {
    ...
    for (const isolateData of coverageMatrix) {
        const isolate = isolateData['Isolate']
        const genePositive = isolateData[sanitizedGene].toString() !== '0'
        const traitPositive = valuesData.isBinary ? valuesData.isolateToBinary[isolate] : null
...
/**
 * Initiate tree metadata heatmaps, ensure clicks on genes update the plot.
 */
