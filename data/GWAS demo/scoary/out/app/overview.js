...
const configPromise = fetch(`app/config.json`)
    ...
    .then(config => {
        ...
        let ths = ''
        let tds = ''
        for (const [key, value] of Object.entries(config.scores)) {
            ths += `<th style="color:${value.color}">${value['marker-html']}</th>`
            tds += `<td>${value.legend}</td>`
        }
        document.getElementById('legend').innerHTML = `<tr>${ths}</tr><tr>${tds}</tr>`

        // Populate legend link
        document.getElementById('legend-link').href = config.legend_url || '#'

        // Populate legend info
        if (config.legend_info) {
            const infoEl = document.getElementById('legend-info')
            infoEl.innerHTML = config.legend_info
